...
sampler = load_distorted_sampler("/mnt/afs/chenyun/TTSFlow/tmp", 15, False)
...
def do_effect(src, steps = 8):
    ...
    # pr, _ = predictor.sample(audio = src.to(torch.float32).to(device), steps = steps, mask= torch.ones(src.shape[0], dtype=torch.bool, device=device))
...
for steps in [4]:
    for ckpt in tqdm(ckpt_list):
        checkpoint = torch.load(f'/mnt/afs/chenyun/TTSFlow/output_wenet/large-03.{ckpt}.pt', map_location="cpu")
