...
def load_clean_sampler(datasets, duration, return_source = False):
    ...
    def sample_item():
        ...
        mel = torchaudio.transforms.MelSpectrogram(sample_rate=config.audio.sample_rate, **stft_params, power=1)(audio) # mel_scale(S**x)
        spec = torchaudio.transforms.InverseMelScale(n_stft=stft_params['n_fft'] // 2 + 1, sample_rate=config.audio.sample_rate)(mel).transpose(0, 1).to(torch.float16)  # S**x
...
def load_effected_sampler(datasets, effect, duration, return_source = False):
    ...
    def sample_item():
        ...
        mel = torchaudio.transforms.MelSpectrogram(sample_rate=config.audio.sample_rate, **stft_params, power=1)(audio) # mel_scale(S**x)
        spec = torchaudio.transforms.InverseMelScale(n_stft=stft_params['n_fft'] // 2 + 1, sample_rate=config.audio.sample_rate)(mel)  # S**x
